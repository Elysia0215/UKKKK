...
import { Calendar, Heart, Layers, GitBranch, Filter, Building } from 'lucide-react';
        ...
        <div className="flex flex-wrap gap-1.5 flex-1">
          {Object.entries(cat1Counts).map(([cat, count]) => {
            ...
            return (
              <button
                key={cat}
                onClick={() => onFilterChange({ ...filterState, category1: cat, category2: '전체', category3: '전체' })}
                className={`px-2.5 py-1 rounded-full border transition font-bold cursor-pointer text-[11px] ${
                  isSelected
        ...
        <div className="flex flex-wrap gap-1.5 flex-1 max-h-32 overflow-y-auto pr-1">
          {Object.entries(cat2Counts).map(([sub, count]) => {
            ...
            return (
              <button
                key={sub}
                onClick={() => onFilterChange({ ...filterState, category2: sub, category3: '전체' })}
                className={`px-2.5 py-1 rounded-full border transition font-bold cursor-pointer text-[11px] ${
                  isSelected
          ...
          <div className="flex flex-wrap gap-1.5 flex-1">
            {Object.entries(cat3Counts).map(([micro, count]) => {
              ...
              return (
                <button
                  key={micro}
                  onClick={() => onFilterChange({ ...filterState, category3: micro })}
                  className={`px-2.5 py-1 rounded-full border transition font-bold cursor-pointer text-[11px] ${
                    isSelected
