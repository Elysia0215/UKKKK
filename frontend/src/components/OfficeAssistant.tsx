...
const PAGE_GUIDE: Record<number, { title: string; purpose: string; steps: string[]; advanced: string[]; footnotes?: Record<string, string> }> = {
  ...
  10: {
    title: '10. 전체 데이터 통계',
    purpose: '상단에서 선택한 정책 대분류와 관련 R&R 팀 조합이 실제 제안 데이터에 어떻게 적용됐는지, 분류·연도·답변·부서·데이터 품질 통계로 한 번에 검증하는 최종 현황판입니다.',
    steps: [
    ...
    if (activeTab === 9) {
      ...
      const scope = selectedDept
        ...
        : selectedDeptGroup
          ? `[${selectedDeptGroup}]`
          : '[전체 대분류 · 전체 관련 R&R 팀]';
      setBubbleText(`${scope} 범위의 전체 데이터 통계입니다. 먼저 현재 표시 건수를 확인한 뒤, 관련 R&R 팀 분포와 1순위 주관부서 TOP의 차이를 비교해 필터 적용을 검증하세요.`);
    }
  ...
  const getFlowSteps = () => {
    ...
    if (!selectedDept && !selectedDeptGroup) {
      return [
        ...
        '10. [전체 데이터 통계] — 상단 필터가 적용된 전체 제안을 분류·연도·답변·부서·품질 통계로 최종 검증합니다.'
      ];
    }
    ...
    return [
      ...
      '10. [전체 데이터 통계] — 현재 범위의 분류·연도·답변·R&R·원문 품질 분포를 한 화면에서 최종 확인합니다.',
