...
const PAGE_GUIDE: Record<number, { title: string; purpose: string; steps: string[]; advanced: string[]; footnotes?: Record<string, string> }> = {
  ...
  9: {
    title: '9. 통합 정책 근거 분석',
    purpose: '현재 대분류와 R&R 팀 범위에서 시민 제안·국민 민원·뉴스·연결 정책·자치구 통계를 같은 대·중·소분류로 묶어, 수요와 정책 공급의 관계를 한 흐름으로 검토하는 화면입니다.',
    steps: [
      '통합 개요에서 제안·민원·뉴스·연결 정책의 근거가 함께 존재하는지 먼저 확인합니다.',
      '기간별 트렌드 → 분류별 근거 → 출처 원문 → 자치구·인프라 → 정책 공백 순서로 근거를 좁혀 봅니다.',
      '각 분석 화면의 [현재 화면 CSV]로 현재 대분류·팀·기간이 적용된 결과를 내려받습니다.'
    ],
    advanced: [
      '전체 근거가 5건 미만이거나 값이 있는 출처가 2종 미만이면 관계를 설명할 수 없어 빈 차트 대신 근거 부족 안내가 표시됩니다.',
      '근거 부족 안내의 [팀 필터 해제]는 대분류는 유지하고 여러 팀의 근거를 합쳐 비교 범위를 넓힙니다.',
      '숫자와 차트 제목 옆 물음표에 마우스를 올리면 집계 기준과 해석 방법을 확인할 수 있습니다.',
      '뉴스 원본에는 현재 발행일 필드가 없으므로 기간 필터에서 뉴스는 연도별 추이가 아닌 누적 근거로 해석합니다.'
    ],
    footnotes: {
      '교차 분석 근거': '제안·민원·뉴스·정책처럼 서로 다른 출처에서 같은 세부 분류를 지지하는 자료',
      '희소 데이터': '건수 또는 출처 종류가 너무 적어 증감·상관·공백 관계를 차트로 판단하기 어려운 범위',
      '연결 정책': '현재 범위의 시민 제안과 내용·분류가 매칭된 기존 서울시 정책의 중복 제거 수'
    }
  },
  10: {
    title: '10. 필터 검증 통계',
    purpose: '상단에서 선택한 정책 대분류와 관련 R&R 팀 조합이 실제 제안 데이터에 어떻게 적용됐는지 검증하는 전용 화면입니다. 외부 근거 교차 분석이 아니라 제안 원본의 분류·연도·답변·부서·품질 상태를 확인합니다.',
    steps: [
      '상단의 [현재 범위]와 [현재 표시 건수]를 먼저 확인해 선택한 대분류·R&R 팀이 의도한 데이터 범위인지 검증합니다.',
      '대분류·관련 R&R 팀·연도·답변 상태·2차 중분류·검토 필요 데이터 카드를 눌러 확대 통계를 확인합니다.',
      '하단 주관부서 TOP 목록에서 관련 R&R 팀 전체가 아니라 실제 1순위 주관부서별 처리 물량을 비교합니다.'
    ],
    advanced: [
      '관련 R&R 팀 분포는 제안별 1·2·3순위 연결 팀을 모두 집계하지만, 주관부서 TOP 목록은 1순위 팀만 집계하므로 두 수치는 서로 다를 수 있습니다.',
      '유사 정책 후보는 확정 대응 정책 수가 아니라 제안에 연결된 검토 후보가 있는 비율입니다.',
      '상세 원문 없음은 통계에는 포함되지만 정책 대조의 근거가 부족한 건이므로 원문 보강 또는 품질 검토가 필요합니다.',
      '고공감 미답변은 공감 150표 이상이면서 아직 답변되지 않은 우선 검토 대상입니다.'
    ],
    footnotes: {
      '현재 표시 건수': '전체 원본 제안 중 상단 대분류와 관련 R&R 팀 조건을 모두 통과해 현재 통계에 포함된 건수',
      '관련 R&R 팀': '제안의 세부 태그와 연결된 1·2·3순위 주관·협조팀 전체',
      '주관부서 TOP': '각 제안의 R&R 순위 중 1순위로 지정된 실제 담당팀만 집계한 순위',
      '유사 정책 후보': '제안과 내용 또는 분류가 유사해 검토 대상으로 연결된 기존 정책'
    }
  }
};
  ...
  useEffect(() => {
    if (activeTab === 9) {
      const scope = selectedDept
        ? `${selectedDeptGroup ? `[${selectedDeptGroup}] · ` : ''}[${selectedDept}]`
        : selectedDeptGroup
          ? `[${selectedDeptGroup}]`
          : '[전체 대분류 · 전체 R&R 팀]';
      setBubbleText(`${scope} 범위의 제안·민원·뉴스·정책을 통합 검토 중입니다. 먼저 출처가 2종 이상 있는지 확인하고, 부족하면 팀 필터를 해제해 범위를 넓혀 보세요.`);
      ...
      const scope = selectedDept
        ...
        : selectedDeptGroup
          ? `[${selectedDeptGroup}]`
          : '[전체 대분류 · 전체 관련 R&R 팀]';
      setBubbleText(`${scope} 범위의 필터 검증 통계입니다. 먼저 현재 표시 건수를 확인한 뒤, 관련 R&R 팀 분포와 1순위 주관부서 TOP의 차이를 비교해 상단 필터 적용을 검증하세요.`);
    }
  }, [activeTab, selectedDept, selectedDeptGroup]);
  ...
  const getFlowSteps = () => {
    ...
    if (!selectedDept && !selectedDeptGroup) {
      return [
        ...
        '9. [통합 정책 근거 분석] — 제안·민원·뉴스·정책·자치구 통계를 같은 분류로 연결해 최종 근거를 교차 검토합니다.',
        '10. [필터 검증 통계] — 상단 필터가 적용된 제안 원본을 분류·연도·답변·부서·품질 통계로 최종 검증합니다.'
      ];
    }
    ...
    return [
      ...
      '9. [통합 정책 근거 분석] — 현재 대분류·팀의 제안·민원·뉴스·정책을 교차 검토하고 화면별 CSV를 저장합니다.',
      '10. [필터 검증 통계] — 현재 범위의 제안 원본 분류·연도·답변·R&R·원문 품질 분포를 한 화면에서 최종 확인합니다.',
