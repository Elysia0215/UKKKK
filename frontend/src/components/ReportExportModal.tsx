...
export const ReportExportModal: React.FC<Props> = ({
  ...
  const generatedReportText = useMemo(() => {
    ...
    if (includeAcademic) {
      ...
      text += `  가. [홍향희·이정화 (2026) 연구 대조 - 국민신문고 저출산 민원 992건 CONCOR 분석 (KJHE 35(1), DOI: http://dx.doi.org/10.5934/kjhe.2026.35.1.1)]:\n`;
      text += `    - 시민 민원의 핵심 쟁점은 단순 현금성 수당 지원보다 '시간제 보육 편의성', '소아 응급 의료 체계', '맞벌이 돌봄 공백 해소' 등 연속적인 보육 인프라 보장임.\n`;
      text += `    - 본 부서 소관 카테고리 연계도: 수집된 보육/돌봄 분야의 시민 수요 강도가 높게 판정되는 것과 일치하여, 실무 우선과제 수립에 타당한 방향성을 지원함.\n`;
      text += `  나. [배기련 외 (2021) 연구 대조 - 저출산 정책과 대중인식 비교 (디지털융복합연구 19(12), KCI: https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART002787528)]:\n`;
      text += `    - 저출산 및 결혼 연계 대중 댓글 25,800건의 분석 결과, 청년층이 느끼는 체감 정책 장벽의 본질은 단순 축하금 수당보다 '주택', '주거', '일자리', '비용' 등 주거 안정성과 일·가정 양립 제도 보장에 있음.\n`;
      text += `    - 본 보고서 연계: 갭 분석 알고리즘에서 도출된 주거 및 노동 환경 관련 정책 공백(Gap) 지수가 높게 나타나는 현 상황을 뒷받침하며, 실질적인 지원 기준 개선 권고의 학술적 근거가 됨.\n`;
      text += `  다. [오신휘·김혜진 (2020) 연구 동향 분석 - 보건사회연구 40(3), KCI: https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART002633196]:\n`;
      text += `    - 저출산 관련 752편의 학술 논문 메타분석을 통해, 텍스트 마이닝을 통한 시민 여론 및 정책 트렌드 분석 기법이 저출산 정책 연구에서 실효적이고 타당한 주류 방법론으로 확립되었음을 실증함.\n`;
      text += `    - 본 보고서 연계: 시민제안과 민원 텍스트를 자연어 처리 및 클러스터링으로 정제하여 의사결정을 지원하는 본 시스템의 방법론적 타당성을 부여함.\n`;
      text += `  라. [성낙일·박선권 (2012) 실증 분석 - 자녀 양육환경과 출산율 (한국인구학 35(2), KCI: https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART001694020)]:\n`;
      text += `    - 전국 232개 시군구의 횡단면 실증분석 결과, 지역별 보육시설의 양적 공급 규모와 육아 편의 환경이 출산율에 유의미한 양(+)의 경제적 효과를 나타냄을 규명함.\n`;
      text += `    - 본 보고서 연계: 자치구별 보육시설 수와 민원/제안 발생 빈도의 상관관계를 갭 지수로 연계 분석하는 본 대시보드의 실측 데이터 지표의 정당성을 뒷받침함.\n`;
      text += `  마. [육아정책연구소 (KICCE, 2023) 보고서 대조 - 영유아 보육기관 수요·공급 현황과 지역별 형평성 진단 (NKIS: https://www.nkis.re.kr/subject_view1.do?otpId=OTP_0000000000013862)]:\n`;
      text += `    - GIS(지리정보시스템) 및 공간 정보를 활용하여 어린이집·유치원 인프라 공급 취약 지역을 선정하고 머신러닝 기반 수요 예측 형평성 모델을 적용함.\n`;
      text += `    - 본 보고서 연계: 서울시 25개 자치구 지도에 보육시설 인프라 수와 시민 수요 갭 분석 데이터를 시각적으로 히트맵 오버레이하는 본 시스템의 시각화 설계 방식을 실증적으로 입증함.\n`;
      text += `  바. [국회예산정책처 (NABO) 보고서 대조 - 저출산 지원정책의 효과성 분석 및 재원배분 방향 연구]:\n`;
      text += `    - 다수 실무 부서 및 유관 사업의 분절화·파편화로 인해 시민의 종합적인 수요 파악 및 체계적인 예산 배분 조정 채널이 부재함을 공식 지적함.\n`;
      text += `    - 본 보고서 연계: 서울시 18개 유관 부서를 유기적으로 연결하고 다차원 5단계 필터 및 R&R 부서 1·2·3순위 자동 라우팅을 제공하는 본 대시보드 도입의 정책적 정당성 근거가 됨.\n`;
      text += `  사. [박미경 (2022) 요구도 분석 - 저출산 정책 요구도 및 우선순위 분석: MZ세대 인식을 중심으로 (KCI: https://www.kci.go.kr/kciportal/po/search/poSereArtiList.kci?sereId=SER000009102)]:\n`;
      text += `    - MZ세대를 대상으로 실증 설문조사를 수행하여 정책 요구도 및 수혜 우선순위를 도출하고 주택 지원 및 일·가정 양립을 최우선 과제로 선정함.\n`;
      text += `    - 본 보고서 연계: 대시보드가 시민 제안의 공감수(vote_score) 및 시급성을 핵심 가중치로 조합하여 산출하는 우선순위 알고리즘 점수의 타당성을 상호 검증(Cross-validation)하는 데 활용됨.\n\n`;
    }
