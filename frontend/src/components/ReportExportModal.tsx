...
export const ReportExportModal: React.FC<Props> = ({
  ...
  const generatedReportText = useMemo(() => {
    ...
    if (includeAcademic) {
      ...
      text += `    - 뉴스 댓글 빅데이터 분석 중 '결혼' 및 '출산'과 동시에 연관지어 출현한 키워드 빈도 분석 결과, '주택', '주거', '일자리', '비용'이 최상위를 차지함.\n`;
      text += `    - 이는 청년층이 결혼과 출산을 실질적으로 결심하는 데 가로막는 최대의 체감 정책 장벽이 단순 현금 수당보다 '출산가구 주거비 부담 완화', '일·가정 양립을 위한 유연근무 제도화', '육아휴직 직장 내 불이익 해소' 등임을 실증적으로 뒷받침함.\n`;
