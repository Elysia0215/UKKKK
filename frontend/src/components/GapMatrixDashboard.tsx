  ...
  if (cat === '보육·돌봄 인프라') {
    return [

      {
        ...
        detail: "전국 232개 시군구의 보육 환경 실증 패널 데이터를 토대로 다중 회귀 분석(Regression Analysis)을 수행한 결과, 지역 내 보육시설 인프라의 양적 공급 규모와 육아 편의 정주 여건의 확충이 지역 합계출산율에 통계적으로 유의미한 양(+)의 경제적 효과(p < 0.05)를 냄을 계량적으로 입증했습니다.",
        implication: "보육시설 인프라 취약 지점과 민원 발생이 높은 자치구에 국공립 어린이집을 최우선적으로 집중 배치해야 합니다.",
        tag: "계량 분석"
      },
      {
        title: "KICCE (2023)",
        url: "https://www.kicce.re.kr",
        detail: "보육 수요-공급 현황 데이터를 활용하여 공간 미스매치 지수(Spatial Mismatch Index) 및 GIS 커널 밀도 분석을 테스트한 결과, 수요(영유아 인구 및 민원) 대비 인프라의 지리적 불균형이 출산율 정체와 직접 연계되는 통계적 선행 인자임을 공간 통계학적으로 규명했습니다.",
        implication: "대시보드가 도출한 자치구별 수요-공급 갭 분석 결과에 준하여, 인프라 사각지대 자치구로 보육 자원을 전략적으로 재배치해야 합니다.",
        tag: "GIS 공간진단"
      }
    ];
  }
    ...
    return [
      {
        title: "배기련 외 (2021)",
        url: "https://www.kci.go.kr",
        detail: "결혼·출산 대중 의견 댓글 25,800건의 비정형 코퍼스 분석 및 감성어 딥마이닝 임베딩 테스트를 거친 결과, 청년층이 체감하는 가장 큰 정책 장벽의 73% 이상이 일자리 안정과 내 집 마련 주거 안정망 결여에 기인함을 실증 검증했습니다.",
        implication: "단순 다자녀 위주의 혜택을 넘어, 1자녀 혹은 무자녀 신혼가구 시절부터 주택 청약 기준을 완화하고 주거 대출 여건을 개선해야 합니다.",
        tag: "소셜 데이터"
      },
      {
        title: "박미경 (2022)",
        url: "https://www.kci.go.kr",
        detail: "미혼·기혼 청년 세대 500명을 대상으로 정책 요구도를 설문조사하고 AHP(계층화분석법) 우선순위 일치성 검증 테스트를 거친 결과, 주거 안정(임대 공급 및 금융 지원)과 일·가정 양립 지원이 타 복지 영역 대비 통계적으로 압도적인 중요도 가중치를 가짐을 규명했습니다.",
        implication: "정책 공급망 진단 및 갭 가설 수립 시 주거 대책과 생활비 지원 간 가중치를 30% 이상 부여하여 예산을 선제 할당하는 알고리즘 설계를 입증합니다.",
        tag: "요구도 분석"
      }
    ];
  }
  ...
  return [
    {
      title: "오신휘·김혜진 (2020)",
      url: "https://www.kci.go.kr",
      detail: "국내 저출산 연구동향 752편의 학술 논문을 메타 데이터 네트워크 분석하고 LDA 토픽 모델링 기법의 예측력을 테스트한 결과, 비정형 시민 여론/제안 데이터를 활용해 갭 분석을 추진하는 것이 전통적 통계 모델의 한계를 극복하는 유의미한 방법론임을 검증했습니다.",
      implication: "시민 제안과 정책 DB를 교차 분석하여 소관 부서를 자동으로 매핑하고 갭 가설을 도출하는 본 대시보드 알고리즘의 신뢰성과 타당성을 최종 보증합니다.",
      tag: "메타 분석"
    },
    {
      title: "NABO 예산정책처",
      url: "https://www.assembly.go.kr",
      detail: "5대 분야 30여 개 저출생 재정 지원 사업의 예산 효율성 및 파편화 지수 테스트를 거친 결과, 부서 및 사업 간 분절성으로 인해 실수혜자가 느끼는 체감 정책 전달률이 40% 이상 하락하는 예산 병목 현상이 실증적으로 나타남을 보고했습니다.",
      implication: "대시보드 상에서 여러 과(과 단위)의 유사 사업들을 대분류 카테고리로 묶어 원스톱으로 확인하고 담당 부서 R&R을 유기적으로 통제하는 컨트롤 타워 도입의 당위성을 입증합니다.",
      tag: "재원 배분"
    }
  ];
};
