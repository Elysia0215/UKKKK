...
| **[Part 3]<br>대시보드 설계** | **Slide 3-1**<br>**Slide 3-1-1**<br>**Slide 3-2**<br>**Slide 3-2-1**<br>**Slide 3-3**<br>**Slide 3-3-1**<br>**Slide 3-4**<br>**Slide 3-4-1**<br>**Slide 3-5** | 화면 ① 종합현황 지도 (기본)<br>화면 ①-1 종합현황 지도 (클릭)<br>화면 ② 8대 수요 필터 (Pruning)<br>화면 ②-1 일괄 답변 기안 모달<br>화면 ③ 갭 매트릭스 (버블차트)<br>화면 ③-1 결재 패널 및 로그 이력<br>화면 ④ 새싹이 오피스 길잡이<br>화면 ⑤ 결측치 복원 시뮬레이터<br>전체 사용자 태스크플로우 | - `slide_3-1_map_default.png` (종합 지도 기본)<br>- `slide_3-1-1_map_clicked.png` (양천구 클릭 연동)<br>- `slide_3-2-1_filter_strike.png` (필터 취소선 UX)<br>- `slide_3-2-2_export_modal.png` (공문 생성 모달)<br>- `slide_3-3-1_gap_matrix.png` (갭 버블차트)<br>- `slide_3-3-2_action_feedback.png` (결재 패널 및 로그)<br>- 윈도우 95 스타일 새싹이 오피스 도움말 패널 스펙 카드<br>- 결측치 복원 + 원문 펼치기 + 체크박스 반영 + CSV 다운로드<br>- 트리 구조로 설계된 전체 태스크플로우 다이어그램 | `20초`<br>`25초`<br>`20초`<br>`25초`<br>`20초`<br>`25초`<br>`30초`<br>`35초`<br>`30초` |
...
> **[스크립트]**
> 안녕하십니까. 서울시 출산·보육 정책 수요 분석 대시보드 **UKKKK**를 발표할 팀입니다.
> UKKKK는 **Unified Key-Knowledge Kit**의 약자로, 서울시 여성가족실 공무원이 시민의 출산·보육 정책 수요를 한눈에 파악하고 신속하게 의사결정할 수 있도록 돕는 통합 백오피스 대시보드입니다.
> 지금부터 10분간, 왜 이 대시보드가 필요한지, 어떤 데이터와 모델로 설계했는지, 그리고 실제 작동하는 화면을 직접 보여드리겠습니다.
...
* **PPT 화면 구성**:
  ...
  - 하단에 돋보기를 든 새싹이 🐶 및 출처 자막 (*국회입법조사처 현안분석 제218호, 성낙일·박선권(2012)*).
  - *새싹이 말풍선*: "일회성 축하금보다 지속 가능한 돌봄 인프라에 시민들이 목말라 있어요! 🔎"
* **시간**: `35초`

> **[스크립트]**
> 저희는 공무원들이 납득할 수 있도록 선행 연구와 국가기관 보고서를 기획의 근거 레이어로 삼았습니다.
> 첫째, 성낙일·박선권(2012)의 국회입법조사처 현안분석입니다. 저출산 대응 재정 사업의 효과성을 분석한 결과, 단순 현금 지원보다 보육 서비스 인프라 확충이 출산율 제고에 더 유의미한 효과가 있음을 입증했습니다.
...
> 중복 및 노이즈가 제거된 상상대로 서울 제안 824건과 국민신문고 실시간 민원 582건, 몽땅정보통 정책 DB 322건, 그리고 네이버 실시간 기사 1,145건을 융합했습니다. 
> 여기에 통계청의 2025년 최종 잠정 속보치인 서울시 합계출산율 0.630명 실측 수치와 구별 인프라 통계까지 병합하여 신뢰도를 기둥으로 세웠습니다.

---

#### Slide 2-2: 전처리 및 AI·규칙 라우팅 모델 (6대 학술·국책연구 실증 연계)
* **PPT 화면 구성**:
  ...
  - **4개 핵심 기술 모듈 & 학술 연구 연계 카드 격자형 배치**:
    ...
    5. **한국어 텍스트마이닝 공유 유틸(`textMining.ts`)**: 조사/어미 제거, 유사어 통합, 동음이의어 안전장치를 프로젝트 전역에서 일관 적용 (크롤링·실시간 수집·프론트엔드 분석 공통)
...
#### Slide 2-3: 우선순위 및 정책 공백도 가중치 산출 공식 (학술 근거 3대 가중치 모델)
* **PPT 화면 구성**:
...
> **[스크립트]**
> 조건에 부합하는 민원들은 8대 대분류별 소관 부서로 자동 라우팅되며, 실무자가 [일괄 답변] 버튼을 누르면 서울시 몽땅정보통의 신청 링크가 연동된 정식 개조식 형태의 한글 공문 기안서 초안이 팝업으로 생성됩니다. 실무자는 단지 복사 버튼을 눌러 내부 결재 시스템에 붙여넣기만 하면 공문 작성이 즉시 완료됩니다.
...
> **[스크립트]**
> 특정 버블을 클릭하면 우측 상세 패널에 AI가 조치 유형과 추천 액션을 제시하고, 바로 아래 Academic Proof 카드가 해당 진단을 뒷받침하는 논문과 실측 데이터 연결고리를 보여줍니다. 실무자 답변 초안은 서울시 상상대로 기존 답변 톤("시민 의견에 서울시가 답합니다" 형식)과 월간 보고서(2026년 5월 보고서) 스타일을 참고하여 행정 공문체로 생성됩니다. 공무원이 승인 결재를 내리면 감사 로그에 누적되는 Human-in-the-loop 피드백 구조입니다.
...
#### Slide 3-4-1: 화면 ⑤ 결측치 복원 시뮬레이터 (5원 데이터 융합 엔진)
* **PPT 화면 구성**:
  - 헤드라인 타이틀: **"90.4% 결측 데이터를 텍스트마이닝으로 되살리다"**
  - **시각화 레이아웃 (새싹이가 찍어준 폴라로이드 사진 테두리 📸)**:
    - `[ slide_3-4-1_missing_data.png 폴라로이드 사진 형태 배치 공간 ]`
  - 핵심 기능 인포그래픽:
    - **한국어 형태소 정규화**: 조사(을/를/가/에) 제거 + 유사어 통합(다둥이→다자녀, 출산율→출산) — 프로젝트 전역 공유 유틸(`textMining.ts`)
    - **동음이의어 안전장치**: '수유'(모유수유 vs 수유동), '방학'(학교방학 vs 방학동) 등 6개 키워드 — 같은 구의 확실한 지명 키워드가 동반 출현해야만 자치구 인정
    - **체크박스 선택 → 데이터베이스 반영**: 복원 성공 건 중 실무자가 검증 후 선택 확정 → 1클릭 반영
    - **원문 펼치기**: 복원 결과 카드에서 원본 제안 전문을 인라인으로 즉시 확인 (외부 링크 이동 없이)
    - **키워드 CSV 다운로드**: TF-IDF 기반 키워드 목록을 대분류/중분류/소분류 포함 CSV로 내보내기 (엑셀 필터링 즉시 가능)
  - 돋보기와 퍼즐 조각을 들고 데이터를 맞추는 새싹이 🐶.
  - *새싹이 말풍선*: "구 미상 804건 중 39건을 텍스트 속 지명으로 자동 복원하고, 나머지는 '서울시 전체(공통)'로 정직하게 분류했어요! 🔍"
* **시간**: `35초`

> **[스크립트]**
> 데이터 품질의 핵심인 결측치 복원 시뮬레이터입니다.
> 상상대로 제안의 원천적 한계는 자치구 미입력, 즉 **'구 미상'이 전체의 90.4%**라는 점입니다.
> 저희는 이를 텍스트마이닝 기반 5원 데이터 융합 엔진으로 해결했습니다. 제안 원문에서 '노원역', '강남구', '잠실' 등 지명 키워드를 추출하되, '수유'나 '방학'처럼 **일상 용어와 동명인 6개 동음이의어는 같은 구의 확실한 지명이 함께 나타날 때만 자치구로 인정**하는 안전장치를 두었습니다.
> 한국어 조사 제거와 유사어 통합 로직은 프로젝트 전역 공유 유틸로 분리하여, 향후 실시간 API 크롤링에서도 일관된 텍스트 정제가 적용됩니다.
> 복원 결과는 실무자가 **원문 펼치기로 즉시 검증**하고, 체크박스로 선택 확정한 뒤 **1클릭으로 데이터베이스에 반영**할 수 있습니다.

---

#### Slide 3-5: 전체 사용자 태스크플로우 및 8대 연동 흐름
* **PPT 화면 구성**:
  - 헤드라인 타이틀: **"대시보드 10대 사용자 연동 흐름 (Task Flow)"**
  - 화면 중앙에 깔끔한 폴더 트리 형태의 10대 핵심 워크플로우 맵 배치:
    ...
    9. **결측치 복원 → 데이터 반영**: 구 미상 제안 일괄 텍스트마이닝 ➔ 복원 결과 원문 펼치기로 검증 ➔ 체크박스 선택 확정 ➔ 1클릭 데이터베이스 반영 ➔ 대시보드 전체 통계 실시간 갱신.
    10. **키워드 CSV 내보내기**: TF-IDF 키워드 탐색기에서 대분류/중분류/소분류 포함 CSV 다운로드 ➔ 엑셀에서 부서별·카테고리별 필터링 즉시 활용.
  - 지휘봉을 들고 8대 플로우 맵을 가리보고 서 있는 대장 새싹이 🐶.
  - *새싹이 말풍선*: "제안 발굴부터 결측치 복원, 공문 복사, 결재 적재까지 10가지 흐름이 유기적으로 연동됩니다! 🏁"
* **시간**: `35초`

> **[스크립트]**
> 시연으로 넘어가기에 앞서, 본 대시보드가 설계한 사용자 태스크플로우, 즉 10대 연동 흐름을 말씀드리겠습니다. 
> 저희 시스템은 개별 탭이 단절되어 작동하지 않고 유기적인 10가지 흐름으로 연계됩니다.
...
> 아홉째, 결측치 복원 시뮬레이터에서 구 미상 제안을 텍스트마이닝으로 일괄 복원한 뒤, 원문 펼치기로 검증하고 체크박스 선택 후 1클릭으로 데이터에 반영합니다.
> 열째, 키워드 탐색기에서 TF-IDF 기반 키워드 목록을 대분류·중분류·소분류 포함 CSV로 내보내 엑셀에서 즉시 필터링 활용할 수 있습니다.
> 이 유기적인 열 가지 행정 워크플로우를 지금부터 실제 라이브 데모로 직접 확인해 보시겠습니다.
...
| **2** | 데이터 분석 정의서 | Word/MD | [데이터분석정의서.md](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/데이터분석정의서.md) | 가중치 연산 수식 및 통계청 인구동향 매핑 정보 |
...
| **7** | 한국어 텍스트마이닝 공유 유틸 | TypeScript | [textMining.ts](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/frontend/src/utils/textMining.ts) | 조사 제거, 유사어 통합, 동음이의어 안전장치, 자치구 추정 엔진 |
| **8** | 결측치 복원 시뮬레이터 | TypeScript | [MissingDataSimulator.tsx](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/frontend/src/components/MissingDataSimulator.tsx) | 5원 데이터 융합 결측치 복원 + 원문 검증 + 데이터 반영 |
