...
* **PPT 화면 구성**:
  ...
  - 3개 핵심 기술 모듈 카드 나란히 배치:
    1. **Hugging Face (`KR-SBERT`)**: `snunlp/KR-SBERT-V40K-klueNLI-augSTS` 모델을 활용한 의미 기반 카테고리 재분류 및 유사 민원 클러스터링.
    2. **Gemini 1.5 Pro / GPT**: 대시보드 내 실시간 뉴스 3초 행정체 요약 및 정책 갭(Gap) 맞춤 권고사항(Recommended Action) 생성.
    3. **설명 가능한 규칙 (R-01~R-05)**: 감사 추적이 가능하도록 설계된 형태소 분석(KoNLPy) 기반 18개 부서 R&R 라우팅 엔진.
  - 지시봉으로 세 개의 AI/규칙 엔진 카드를 짚어주고 있는 새싹이 🐶.
  - *새싹이 말풍선*: "허깅페이스 SBERT와 제미나이 LLM, 규칙 기반 라우팅으로 정확성과 설명 가능성을 모두 잡았어요! 👨‍🏫"
* **시간**: `45초`

> **[스크립트]**
> 수집된 텍스트 데이터의 분석 및 라우팅에는 고도화된 AI 모델과 명확한 규칙 엔진을 병행 탑재했습니다.
> 먼저, 키워드 매칭의 중복 한계를 극복하기 위해 허깅페이스의 한국어 임베딩 모델인 KR-SBERT를 도입하여 시민 제안의 의미론적 유사도 벡터 분석을 진행하고 카테고리 재분류 및 유사 민원 자동 군집을 완료했습니다. 
> 또한, 대시보드의 실시간 뉴스 요약과 의사결정 권고사항 생성에는 제미나이와 GPT API를 탑재했습니다. 
> 마지막으로, 실제 행정 배정은 투명성과 설명 가능성이 중요하므로 형태소 분석 기반의 5대 R&R 라우팅 규칙을 커스텀 설계하여 서울시 18개 부서 매칭을 정확히 완성했습니다.
