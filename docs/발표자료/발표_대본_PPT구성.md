...
| **[Part 3]<br>대시보드 설계** | **Slide 3-1**<br>**Slide 3-1-1**<br>**Slide 3-2**<br>**Slide 3-2-1**<br>**Slide 3-3**<br>**Slide 3-3-1** | 화면 ① 종합현황 지도 (기본)<br>화면 ①-1 종합현황 지도 (클릭)<br>화면 ② 8대 수요 필터 (Pruning)<br>화면 ②-1 일괄 답변 기안 모달<br>화면 ③ 갭 매트릭스 (버블차트)<br>화면 ③-1 결재 패널 및 로그 이력 | - [slide_3-1_map_default.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-1_map_default.png)<br>- [slide_3-1-1_map_clicked.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-1-1_map_clicked.png)<br>- [slide_3-2-1_filter_strike.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-2-1_filter_strike.png)<br>- [slide_3-2-2_export_modal.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-2-2_export_modal.png)<br>- [slide_3-3-1_gap_matrix.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-3-1_gap_matrix.png)<br>- [slide_3-3-2_action_feedback.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-3-2_action_feedback.png) | `20초`<br>`25초`<br>`20초`<br>`25초`<br>`20초`<br>`25초` |
...
* **PPT 화면 구성**:
  ...
  - 4개 핵심 기술 모듈 카드 격자형 배치:
    ...
    3. **Gemini 1.5 Pro / GPT-4o (LLM)**: 실시간 뉴스 3초 행정체 요약 및 정책 갭 권고사항 생성 API.
    4. **형태소 분석(KoNLPy) & 규칙 엔진**: 감사 대비 설명 가능성 확보를 위한 규칙 기반 18개 부서 R&R 라우팅.
  - 지시봉으로 4대 기술 카드를 가리키는 늠름한 새싹이 🐶.
  - *새싹이 말풍선*: "허깅페이스 SBERT, NMF, LLM과 규칙 엔진을 최적으로 조합한 하이브리드 설계입니다! 👨‍🏫"
* **시간**: `45초`
...
> 셋째, 실시간 요약과 추천 액션 생성에는 제미나이와 GPT API를 탑재했습니다.
> 마지막으로, 부서 배정은 감사의 투명성을 위해 형태소 분석 기반의 5대 규칙 엔진을 구축하여 서울시 18개 부서 매칭을 오차 없이 수행했습니다.
...
* **PPT 화면 구성**:
  ...
  - 대시보드의 지도 영역을 중심으로 한 고해상도 전체 화면 캡처 이미지.
  - **이미지 삽입 경로**: [slide_3-1_map_default.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-1_map_default.png)
  - 자치구 경계선 위에 서서 지도를 가리키고 있는 새싹이 🐶.
  - *새싹이 말풍선*: "서울시 저출생 3대 KPI와 25개 자치구 지도가 첫 화면에 노출됩니다! 🗺️"
* **시간**: `20초`
...
* **PPT 화면 구성**:
  ...
  - 특정 자치구(양천구)를 클릭하여, 상단 카드의 출생아 수 및 인프라 통계가 동기화되어 필터링된 화면 캡처.
  - **이미지 삽입 경로**: [slide_3-1-1_map_clicked.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-1-1_map_clicked.png)
  - 양천구 위에 깃발을 꽂고 서 있는 새싹이 🐶.
  - *새싹이 말풍선*: "양천구를 클릭하면 대시보드 전체 통계가 동기화되어 바뀝니다! 🗺️"
* **시간**: `25초`
...
* **PPT 화면 구성**:
  ...
  - 8대 분야 카테고리 필터 영역에서 특정 분야 선택 시 유효 데이터가 없는 분야 버튼들이 실시간 취소선 및 흐림(Fade) 처리된 클로즈업 캡처 이미지.
  - **이미지 삽입 경로**: [slide_3-2-1_filter_strike.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-2-1_filter_strike.png)
  - 취소선 버튼 옆에서 꼬리를 흔드는 새싹이 🐶.
...
* **PPT 화면 구성**:
  ...
  - [일괄 답변] 클릭 시 몽땅정보통 신청 링크 등이 결합되어 완성된 정식 개조식 한글(HWP) 공문서 양식 팝업 창의 클로즈업 캡처 이미지.
  - **이미지 삽입 경로**: [slide_3-2-2_export_modal.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-2-2_export_modal.png)
  - 문서 뭉치 위에서 경례하고 있는 새싹이 🐶.
  - *새싹이 말풍선*: "몽땅정보통 신청 링크가 포함된 한글 공문 기안서 초안이 1클릭으로 생성돼요! 📄"
* **시간**: `25초`
...
* **PPT 화면 구성**:
  ...
  - 공급 공백(Gap)과 시급성을 기준으로 4분면에 버블로 매핑된 의사결정 갭 분석 매트릭스 영역의 고해상도 캡처 이미지.
  - **이미지 삽입 경로**: [slide_3-3-1_gap_matrix.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-3-1_gap_matrix.png)
  - 버블 차트를 가리키며 서 있는 새싹이 🐶.
  - *새싹이 말풍선*: "공급 공백과 시급성을 기준으로 4분면 버블차트에 우선순위가 분석됩니다! 📊"
* **시간**: `20초`
...
* **PPT 화면 구성**:
  ...
  - 우측 Sticky 고정 상세 결재 패널 및 하단 결재 이력 로그 테이블 영역의 캡처 이미지.
  - 패널 최하단 결재 승인 버튼 아래 여백에 조용히 엎드려 있는 새싹이 🐶.
  - **이미지 삽입 경로**: [slide_3-3-2_action_feedback.png](file:///Users/parkcy/Desktop/sesac_pjt/UKKKK/docs/발표자료/이미지/slide_3-3-2_action_feedback.png)
  - *새싹이 말풍선*: "AI 추천을 검토한 승인 피드백이 전송되면 감사 로그에 누적 보존됩니다! 🐾"
* **시간**: `25초`
...
* **PPT 화면 구성**:
  ...
  - 좌측에서 우측으로 향하는 2단계 심플 화살표 로드맵.
    - **Step 1 (단기)**: `포트 포워딩 최적화` (OpenAI/Gemini 연동 모듈 충돌 및 Railway 502 에러 해결 완료)
...
> 단기적으로는 OpenAI 및 제미나이 API 연동 컨테이너 가동 시 발생했던 포트 충돌과 Railway 배포 서버의 502 Bad Gateway 에러를, 포트 포워딩 라우팅 분리 및 의존성 주입 최적화 작업을 통해 완벽하게 선제 해결하여 프로덕션 안정성을 100% 확보했습니다. 
