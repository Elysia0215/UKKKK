...
#### Slide 3-5: 전체 사용자 태스크플로우 (Task Flow)
* **PPT 화면 구성**:
  - 헤드라인 타이틀: **"15초 행정 단축을 위한 태스크플로우"**
  - 화면 중앙에 깔끔한 폴더 트리 구조의 사용자 Flowchart 배치:
    ```
    [1. 부서 선택: 저출생사업2팀 지정]
          │
          ▼
    [2. 종합 수요 분석] ── (자치구 지도 클릭: 양천구 선택)
          │
          ▼
    [3. 정책 우선순위] ── (보육·돌봄 인프라 필터링) ── (1클릭 일괄 답변 기안문 생성)
          │
          ▼
    [4. 의사결정 갭 분석] ── (4분면 버블차트 선택) ── (AI 추천 액션 승인 피드백 전송)
          │
          ▼
    [5. 감사 결재 로그 적재] (Human-in-the-loop 환류 완료)
    ```
  - 지휘봉을 들고 플로우차트 앞에 서 있는 대장 새싹이 🐶.
  - *새싹이 말풍선*: "다섯 단계만 거치면 시민 제안 발굴부터 공문 복사, 결재 적재까지 끝납니다! 🏁"
* **시간**: `30초`

> **[스크립트]**
> 시연으로 넘어가기에 앞서, 본 대시보드가 설계한 업무 흐름, 즉 태스크플로우를 트리 구조로 보여드리겠습니다.
> 첫째, 실무자가 부서를 등록하면 새싹이의 추천 동선이 활성화됩니다.
> 둘째, 종합 지도에서 특정 구의 현황을 파악하고,
> 셋째, 우선순위 탭에서 미답변 제안을 필터링해 한글 기안문을 1클릭 복사합니다.
> 넷째, 갭 분석표에서 버블 차트로 미스매칭을 판정한 후 AI 액션을 승인 전송하면,
> 다섯째, 결재 감사 로그에 실시간 적재되어 모든 행정이 단 15초 만에 완료됩니다. 이제 실제 시연으로 확인해 보시겠습니다.
