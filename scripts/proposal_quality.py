...
def classify_birth_policy_category(title: Any, content: Any = "") -> tuple[str, str, str]:
    ...
    if has_any(text, ("한부모", "미혼모", "미혼부", "위기임산부", "결혼이민", "결혼이주", "이주여성", "다문화가족")):
        return "취약·다양가족 사각지대", "취약·다양가족 지원", "한부모·다문화·위기임산부"
    # 어린이집·현금지원 등을 기존 정책의 실패 사례로 언급하더라도
    # 실제 요구가 무급 재생산 노동의 인정, 경력·연금 공백 보완이라면
    # 보육시설 공급이 아니라 부모 노동·돌봄 병행 의제로 분류한다.
    if has_any(text, (
        "재생산 노동", "무임금 육아", "무급 육아", "육아 노동",
        "가사 노동", "가사노동", "경력 공백", "연금 공백",
        "독박 부담", "독박육아",
    )):
        return (
            "일·가정 양립·부모 노동",
