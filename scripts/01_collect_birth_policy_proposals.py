...
def classify_birth_policy_category(title: str, content: str = "") -> Tuple[str, str, str]:
    """
    기존 8개 대분류 기준으로 대분류·중분류·소분류를 정밀 부여한다.
    사용자 정의 5대 분류 보정 룰 적용:
    1. 임산부 + (교통|지하철|버스|좌석|배려석|뱃지|태그) -> 주거·교통·도시생활환경 > 임산부 이동권 > 임산부 교통배려
    2. 양육지원/이용대상/신청/서비스/안내 -> 정보·상담·교육·거버넌스 > 양육지원 서비스 접근성 > 신청·이용대상 기준 개선
    3. 유아숲/체험/놀이터/키즈카페/휴게실 -> 보육·돌봄 인프라 > 아동 놀이·체험공간 > 유아·아동 체험공간
    4. 저출산/저출생 + 정책/아이디어/제안/홍보 -> 정보·상담·교육·거버넌스 > 저출산 정책 일반 > 인식개선·정책제안
    5. 신혼부부 + 이자지원/대출/주택/전세/월세 -> 주거·교통·도시생활환경 > 출산가구 주거 > 신혼부부·출산가구 주거지원
    """
    text = normalize_text(f"{title} {content}")

    # 룰 1. 임산부 이동권 / 배려석 / 뱃지 / 대중교통
    if any(kw in text for kw in ["임산부석", "배려석", "뱃지", "태그", "임산부 좌석", "임산부 교통", "임산부 지하철", "임산부 버스"]):
        return "주거·교통·도시생활환경", "임산부 이동권", "임산부 교통배려"
    if "임산부" in text and any(kw in text for kw in ["교통", "지하철", "버스", "좌석", "배려석", "이동", "안내"]):
        return "주거·교통·도시생활환경", "임산부 이동권", "임산부 교통배려"

    # 룰 5. 신혼부부 / 출산가구 주거 및 이자지원
    if "신혼부부" in text or "신생아 특례대출" in text:
        if any(kw in text for kw in ["주거", "주택", "월세", "전세", "대출", "이자", "임차보증금", "아파트"]):
            return "주거·교통·도시생활환경", "출산가구 주거", "신혼부부·출산가구 주거지원"

    # 룰 3. 유아숲 / 체험 / 놀이터 / 키즈카페 / 유아휴게실
    if any(kw in text for kw in ["유아숲", "키즈카페", "유아휴게실", "영유아쉼터", "체험원", "놀이터", "놀이공간"]):
        return "보육·돌봄 인프라", "아동 놀이·체험공간", "유아·아동 체험공간"
    if "체험" in text and any(kw in text for kw in ["유아", "아동", "어린이", "가족"]):
        return "보육·돌봄 인프라", "아동 놀이·체험공간", "유아·아동 체험공간"

    # 룰 4. 저출산 / 저출생 정책 일반 및 인식개선
    if any(kw in text for kw in ["저출산", "저출생", "고령화"]):
        if any(kw in text for kw in ["정책", "대책", "제안", "아이디어", "홍보", "인식개선", "토론", "담론"]):
            return "정보·상담·교육·거버넌스", "저출산 정책 일반", "인식개선·정책제안"

    # 룰 2. 양육지원 서비스 접근성 / 이용대상 / 신청기준
    if any(kw in text for kw in ["양육지원", "이용대상", "신청기준", "서류제출", "규제개선", "행정제도"]):
        return "정보·상담·교육·거버넌스", "양육지원 서비스 접근성", "신청·이용대상 기준 개선"

    # 기본 정밀 분류
    if any(kw in text for kw in ["난임", "난임시술", "난임주사", "PGT", "유전자검사", "AMH", "난자동결"]):
    ...
    if any(kw in text for kw in ["출산지원금", "출생축하", "출산장려금", "첫만남", "신생아"]):
        return "출산·산후 초기지원", "출산가구 초기지원", "출산지원금·출생축하"

    if any(kw in text for kw in ["초등돌봄", "키움센터", "방과후", "방학돌봄", "초등학생"]):
    ...
    if any(kw in text for kw in ["다자녀", "다둥이", "2자녀", "세자녀", "양육", "양육비", "양육수당"]):
