...
### Ver2 검증 결과 및 다음 수집 보정 기준

확장 키워드 및 문맥 조합 조건을 적용한 결과, 기존 704건에서 824건으로 출산·양육 후보 데이터가 확대되었습니다. 다만 검증용 샘플 확인 결과, 일부 일반 아동 안전·공공시설 제안이 포함될 수 있어 `아이/아동/어린이`와 같은 범용 키워드는 추가 양육 맥락 조건을 요구하도록 필터링 기준을 보완했습니다.

다음 수집에서는 조합 조건을 무작정 넓히기보다 `아픈아이돌봄`, `병아동돌봄`, `시간제보육`, `수유실`, `기저귀갈이대`, `산후우울`, `가임력검사`, `소아응급`, `달빛어린이병원`, `부모급여`, `아동수당` 등 출산·양육 직접 생활 키워드를 우선 사용합니다.

보정 룰은 다음과 같습니다.

- `안전`, `보호구역`, `CCTV`, `통학로`, `스쿨존` 조합은 `어린이집`, `유치원`, `영유아`, `등하원`, `돌봄`, `양육`, `부모` 중 하나가 함께 있을 때만 최종 후보로 유지합니다.
- `도서관`, `문화센터`, `체육센터`, `공공시설` 조합은 `수유실`, `기저귀교환대`, `유아휴게실`, `유모차`, `영유아`, `부모교육`, `장난감도서관` 중 하나가 함께 있을 때만 유지합니다.
- `학교폭력`, `청소년`, `학교`, `체육센터`, `스포츠센터`, `생활SOC` 중심 제안은 출산·양육 직접 키워드가 없으면 제외로그로 보냅니다.
